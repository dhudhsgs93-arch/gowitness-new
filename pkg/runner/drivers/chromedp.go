...
func getChromedpAllocator(opts runner.Options) (*browserInstance, error) {
	...
	if opts.Chrome.WSS == "" {
		...
		allocOpts := append(chromedp.DefaultExecAllocatorOptions[:],
			...
			// A single --disable-features switch: Chrome keeps only the last
			// occurrence of a duplicated switch, so two separate calls would
			// drop all but the last feature list.
