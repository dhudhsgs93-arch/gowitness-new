...
const ScreenshotDetailPage = () => {
  ...
  const commentTimer = useRef<ReturnType<typeof setTimeout>>();
  ...
  // Sync review state when detail loads
  useEffect(() => {
    if (detail?.review) {
      setReviewStatus(detail.review.status || '');
  ...
  const saveReview = useCallback(async (status: string, comment: string) => {
    try {
      await api.post('reviewSet', { status, comment }, { id: currentId });
    } catch {
  ...
  const handleStatusClick = (key: string) => {
    const newStatus = reviewStatus === key ? '' : key;
    setReviewStatus(newStatus);
    saveReview(newStatus, reviewComment);
  };
            ...
            <CardContent className="space-y-3">
              <div className="flex gap-2 flex-wrap">
                {REVIEW_STATUSES.map(s => {
                  const Icon = s.icon;
                  const isActive = reviewStatus === s.key;
                  return (
                    <Button
                      key={s.key}
                      ...
                      className={cn("h-8", isActive && s.bg)}
                    >
                      <Icon className={cn("w-4 h-4 mr-1", s.color)} />
                      {s.label}
                    </Button>
                  );
                })}
